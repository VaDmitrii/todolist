...
@pytest.mark.django_db
def test_boards_list(client, user, board):
    expected_response = {
        "count": 1,
        "next": None,
        "previous": None,
    ...
    response_list = client.get('/goals/board/list')

    assert response_list.status_code == 200
    assert response_list.data[0]["id"] == expected_response['results'][0]["id"]
    assert response_list.data[0]["title"] == expected_response['results'][0]["title"]
...
@pytest.mark.django_db
def test_create_category(client, board_participant, goal_category):
    expected_response = {
        "id": 2,
    ...
    assert response.data["id"] == expected_response["id"]
    assert response.data["title"] == expected_response["title"]
    assert response.data["user"] == expected_response["user"]
...
@pytest.mark.django_db
def test_create_goal(client, user, goal):
    expected_response = {
        "id": 2,
